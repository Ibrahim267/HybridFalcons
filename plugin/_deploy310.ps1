# ============================================================
#  FitDeveloper 3.1.0 deploy - installs into Android Studio
...
$zip    = "$repo\build\distributions\fitdeveloper-plugin-3.1.0.zip"
...
$report = "$repo\_deploy310_log.txt"

function Log($m) { Write-Output $m; Add-Content -Path $report -Value $m }

Set-Content -Path $report -Value "=== FitDeveloper 3.1.0 deploy $(Get-Date) ==="
...
foreach ($cfg in @($asCfg, $iuCfg)) {
  ...
  tar -xf $zip -C $cfg
  $jar = "$cfg\fitdeveloper-plugin\lib\fitdeveloper-plugin-3.1.0.jar"
  if (Test-Path $jar) { Log ("seeded OK: " + $jar) }
  else { Log ("SEED FAILED - jar not at " + $jar) }
}

# ---- 4. desktop zip for manual Install-from-Disk ----
Copy-Item $zip "$env:USERPROFILE\Desktop\fitdeveloper-plugin-3.1.0.zip" -Force
