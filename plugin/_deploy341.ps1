# ============================================================
#  FitDeveloper 3.4.1 deploy - installs into Android Studio
#  AND IntelliJ IDEA, then relaunches both with the test project
#  (readable progress bar: ink caption on orange fill, no more
#   theme-blue-on-gray)
# ============================================================
$ErrorActionPreference = 'Continue'
$repo   = 'C:\Users\loq\Documents\hackathons\JetBrains\FitDeveloper\plugin'
$zip    = "$repo\build\distributions\fitdeveloper-plugin-3.4.1.zip"
$testPrj= 'C:\Users\loq\Documents\hackathons\JetBrains\FitDeveloper-IDE-test'
$asCfg  = "$env:APPDATA\Google\AndroidStudio2024.3.2\plugins"
$iuCfg  = "$env:APPDATA\JetBrains\IntelliJIdea2026.2\plugins"
$report = "$repo\_deploy341_log.txt"

function Log($m) { Write-Output $m; Add-Content -Path $report -Value $m }

Set-Content -Path $report -Value "=== FitDeveloper 3.4.1 deploy $(Get-Date) ==="
...
$hasHook    = ($entries | Select-String 'FitDeveloperDynamicHook.class') -ne $null
$hasQr      = ($entries | Select-String 'QrCode.class') -ne $null
$hasHub     = ($entries | Select-String 'BreakFlashHub.class') -ne $null
Log ("jar classes present: engine=" + $hasEngine + " factory=" + $hasFactory + " lock=" + $hasLock + " hook=" + $hasHook + " qr=" + $hasQr + " hub=" + $hasHub)
if (-not ($hasEngine -and $hasFactory -and $hasLock -and $hasHook -and $hasQr -and $hasHub)) { Log 'ABORT - stale zip'; exit 1 }
...
foreach ($cfg in @($asCfg, $iuCfg)) {
  ...
  tar -xf $zip -C $cfg
  $jar = "$cfg\fitdeveloper-plugin\lib\fitdeveloper-plugin-3.4.1.jar"
  if (Test-Path $jar) { Log ("seeded OK: " + $jar) }
  else { Log ("SEED FAILED - jar not at " + $jar) }
}

# ---- 4. desktop zip for manual Install-from-Disk ----
Copy-Item $zip "$env:USERPROFILE\Desktop\fitdeveloper-plugin-3.4.1.zip" -Force
...
Log 'Android Studio launching with FitDeveloper-IDE-test'
if ($idea64 -and (Test-Path $idea64)) {
  Start-Process $idea64 -ArgumentList ('"' + $testPrj + '"')
  Log 'IntelliJ IDEA launching with FitDeveloper-IDE-test'
} else {
