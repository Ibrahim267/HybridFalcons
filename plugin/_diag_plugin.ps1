...
Write-Output '--- all crunchguard plugin-load lines (last 12) ---'
if (Test-Path $log) {
  $l = Get-Content $log
  Write-Output ('log size: ' + (Get-Item $log).Length + 'B, lines: ' + $l.Count)
  $l | Select-String -SimpleMatch 'com.crunchguard.plugin' | Select-Object -Last 12 | ForEach-Object { $_.Line }
  Write-Output '--- crunchguard runtime markers (last 8) ---'
  $l | Select-String -SimpleMatch '[CrunchGuard]' | Select-Object -Last 8 | ForEach-Object { $_.Line }
...
Write-Output '--- crunchguard jar check (any version) ---'
Get-ChildItem "$cfg\plugins\crunchguard-plugin\lib" -ErrorAction SilentlyContinue | ForEach-Object { '  ' + $_.Name + '  ' + $_.Length + 'B' }
