# ============================================================
#  FitDeveloper 3.3.0 deploy - installs into Android Studio
...
$zip    = "$repo\build\distributions\fitdeveloper-plugin-3.3.0.zip"
...
Set-Content -Path $report -Value "=== FitDeveloper 3.3.0 deploy $(Get-Date) ==="
...
foreach ($cfg in @($asCfg, $iuCfg)) {
  ...
  tar -xf $zip -C $cfg
  $jar = "$cfg\fitdeveloper-plugin\lib\fitdeveloper-plugin-3.3.0.jar"
  if (Test-Path $jar) { Log ("seeded OK: " + $jar) }
  else { Log ("SEED FAILED - jar not at " + $jar) }
}

# ---- 4. desktop zip for manual Install-from-Disk ----
Copy-Item $zip "$env:USERPROFILE\Desktop\fitdeveloper-plugin-3.3.0.zip" -Force
