# ============================================================
#  FitDeveloper 3.3.1 deploy - installs into Android Studio
#  AND IntelliJ IDEA, then relaunches both with the test project
#  (new: Break scan screen setting - Both / Plugin only / Browser only)
# ============================================================
$ErrorActionPreference = 'Continue'
$repo   = 'C:\Users\loq\Documents\hackathons\JetBrains\FitDeveloper\plugin'
$zip    = "$repo\build\distributions\fitdeveloper-plugin-3.3.1.zip"
...
$report = "$repo\_deploy330_log.txt"

function Log($m) { Write-Output $m; Add-Content -Path $report -Value $m }

Set-Content -Path $report -Value "=== FitDeveloper 3.3.1 deploy $(Get-Date) ==="
...
Log ("jar classes present: engine=" + $hasEngine + " factory=" + $hasFactory + " lock=" + $hasLock + " hook=" + $hasHook + " qr=" + $hasQr)
if (-not ($hasEngine -and $hasFactory -and $hasLock -and $hasHook -and $hasQr)) { Log 'ABORT - stale zip'; exit 1 }
...
foreach ($cfg in @($asCfg, $iuCfg)) {
  ...
  tar -xf $zip -C $cfg
  $jar = "$cfg\fitdeveloper-plugin\lib\fitdeveloper-plugin-3.3.1.jar"
  if (Test-Path $jar) { Log ("seeded OK: " + $jar) }
  else { Log ("SEED FAILED - jar not at " + $jar) }
}

# ---- 4. desktop zip for manual Install-from-Disk ----
Copy-Item $zip "$env:USERPROFILE\Desktop\fitdeveloper-plugin-3.3.1.zip" -Force
